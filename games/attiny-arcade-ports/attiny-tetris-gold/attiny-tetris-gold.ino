/*
   ATtiny Tetris Gold - Multi Button (Tiny Handheld port 2020 Aug)
...
// PB1 pin change interrupt (for wake up)
ISR(PCINT0_vect) {
}
...
void loop() {
  ...
  if (sChange == 0) {
    ...
    uint8_t sng = 50; // (ghost == 1) ? 50 : 25;
    for (uint8_t i = 0; i < sng; i++) {
      // reduce note duration by 2 to speed up music
      int note = pgm_read_word_near(&(music[i][0]));
      int duration = pgm_read_word_near(&(music[i][1])) >> 1;
      soundPlay(note, duration);
    }
    ssd1306_fillscreen(0x00);
    playTetris();
  }
