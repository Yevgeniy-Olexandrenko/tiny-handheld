...
ONE:;
	...
	while (1)
	{
		if (VARIABLE.Frame % 8 == 0)
		{
			...
			if ((VARIABLE.launch == 0) && (digitalRead(1) == LOW))
			{
				VARIABLE.launch = 1;
			}
...
#define VAl01 (VAR->LEVEL - (VAl10 * 10))
	...
	{
		return (pgm_read_byte(&DIGITAL[(X - 117) + (VAl01 * 7)]));
	}
	return 0x00;
}
...
uint8_t Block(uint8_t X, uint8_t Y, GROUPE *VAR)
{
	...
	if ((X >= 67) && (X < 97) && (Y >= 1) && (Y <= 6))
	{
		...
		{
			return pgm_read_byte(&BLOCKREFLECT[((X - 67) - (XValue * 6)) + (VAR->ANIMREFLECT * 6)]) | pgm_read_byte(&BLOCK[((X - 67) - (XValue * 6)) + ((VAR->BlocsGrid[(Y - 1)][XValue]) * 6)]);
		}
...
#define BALLYPOS (VAR->Ballypos - 1)
	...
	if (VAR->BALLyDecal == 0)
	{
		if (Y == VAR->Ypos)
		{
			return (pgm_read_byte(&BALL[(X - uint8_t(BALLXPOS))]));
		}
	}
	else
	{
		...
		if (Y == VAR->Ypos)
		{
			return SplitSpriteDecalageY(DECAL, pgm_read_byte(&BALL[(X - uint8_t(BALLXPOS))]), 1);
		}
		if (Y == (VAR->Ypos) + 1)
		{
			return SplitSpriteDecalageY(DECAL, pgm_read_byte(&BALL[(X - uint8_t(BALLXPOS))]), 0);
		}
	}
	return 0x00;
}
