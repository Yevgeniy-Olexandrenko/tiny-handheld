 ...
 *
 ...
 Comments from Billy Cheung
  ...
  B3 - OLED SCL
  B4 - OLED SDA
  B5/A3 - Reset - Fire button - tie to a voltage divider , or press both left and right button to fire.

  
  I/O for Tiny Joypad that this version has been modiifed for
  
  B0      OLED SDA
  B1      Fire button to fire
  B2      OLED SCL
...
void loop() { 
  ...
  ssd1306_char_f6x8(0, 2, " UP   : UFO");
  ssd1306_char_f6x8(0, 6, " DOWN : BREAKOUT"); 
  gameNbr = 0;
  fireCount = 0;
  while (gameNbr == 0)  {
  ...
  if (sChange == 0) {
    ...
    if (gameNbr==1) { playBreakout(); top=topScoreB;} else { playUFO(); top=topScoreU;}
